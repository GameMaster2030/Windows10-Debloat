...
# Rewrite of the questions.
#$Ask = select-menuitem -prompt "Select action:" -menutext '&Yes = Start debloat `n &No = Revert changes `n Cancel = Stop script' -default "Yes"
$Ask = select-menuitem -prompt "Do you want to start the program? (No is reverting changes)" -menutext 'ync' -default "Yes"
...
if($($Ask.ToLower()) -eq "cancel") {
	exit
}
if($($Ask.toLower()) -eq "no") {
    ...
    $PromptR1 = select-menuitem -prompt "Do you want to set Edge as default PDF viewer?" -menutext 'yn' -default "No"
	if($($PromptR1.toLower()) -eq "yes") {
        Enable-EdgePDF
        Write-Host "Edge will no longer be disabled from being used as the default Edge PDF viewer."
    }
	if($($PromptR1.toLower()) -eq "no") {
        Write-Host "You have chosen to keep the setting that disallows Edge to be the default PDF viewer."
    }
    #Prompt asking if you'd like to reboot your machine
    $PromptR2 = select-menuitem -prompt "Do you want to restart your computer?" -menutext 'yn' -default "No"
	if($($PromptR2.toLower()) -eq "yes") {
...
$Prompt2 = select-menuitem -prompt "Disable Edge as default PDF viewer?" -menutext 'yn' -default "No"
if($($Prompt2.toLower()) -eq "yes") {
	Stop-EdgePDF
    Write-Host "Edge will no longer take over as the default PDF viewer."
}
if($($Prompt2.toLower()) -eq "no") {
	Write-Host "You chose not to stop Edge from taking over as the default PDF viewer."
}
$Prompt3 = select-menuitem -prompt "Uninstall Onedrive?" -menutext 'yn' -default "No"
if($($Prompt3.toLower()) -eq "yes") {
	UninstallOneDrive
    Write-Host "OneDrive is now removed from the computer."
}
if($($Prompt3.toLower()) -eq "no") {
	Write-Host "You have chosen to skip removing OneDrive from your machine."
}
$Prompt4 = select-menuitem -prompt "Unpin tiles in start menu?" -menutext 'yn' -default "No"
if($($Prompt4.toLower()) -eq "yes") {
	UnpinStart
    Write-Host "Start Apps unpinned."
}
if($($Prompt4.toLower()) -eq "no") {
	Write-Host "Apps will remain pinned to the start menu."
}
$Prompt5 = select-menuitem -prompt "Do you want to restart your computer?" -menutext 'yn' -default "No"
if($($Prompt5.toLower()) -eq "yes") {
	Write-Host "Unloading the HKCR drive..."
                Remove-PSDrive HKCR 
                Start-Sleep 1
                Write-Host "Initiating reboot."
